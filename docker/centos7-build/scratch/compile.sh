...
working_directory=$thisdir/install-Molpro # careful! if this directory already exists it will be completely destroyed first
...
export CXX_MPI_VENDOR=MPItrampoline
./configure FC=gfortran CXX=mpicxx --enable-mpp=ga FCFLAGS="-g" CFLAGS="-g" CXXFLAGS="-DMPIO_USES_MPI_REQUEST -g" CPPFLAGS="-I${working_directory}/include -I${working_directory}/include/eigen3" --prefix=$prefix LDFLAGS="-static-libstdc++ -static-libgcc -L${working_directory}/lib -Wl,-rpath,${MKLROOT}/lib/intel64 -Wl,-rpath,${GCC_ROOT}/lib64" LAUNCHER='mpiexec -n %n %x' --bindir=${prefix}/bin
